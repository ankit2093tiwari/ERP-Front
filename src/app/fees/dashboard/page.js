...
const DashboardFee = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        // 1. Total Students
        const studentRes = await getTotalStudentsCount(selectedSessionId);
        setStudentCount(studentRes?.data?.totalStudents || 0);

        // 2. Recent Fee Entries
        const recentRes = await getLimitedFeeEntries(5, selectedSessionId);
        setRecentFeeEntries(recentRes?.feeEntries || []);

        // 3. Expected Yearly Fee
        const expectedRes = await getExpectedYearlyFees(selectedSessionId);
        const expectedAmount = expectedRes?.totalExpectedAmount || 0;
        setTotalFeeAmount(expectedAmount);

        // 4. All Fee Entries (for charts + collected/pending)
        const feeRes = await getAllFeeEntries(selectedSessionId);
        const entries = feeRes?.feeEntries || [];
        ...
        entries.forEach((entry) => {
          const inst =
            entry.installment_name?.installment_name || "Unknown";
        ...
        const filteredCategories = orderedMonths.filter(
          (m) => m in totalsByInstallment
        );
        const chartData = filteredCategories.map(
          (m) => totalsByInstallment[m]
        );
        ...
        const totalCollected =
          admissionTotal + annualTotal + tuitionTotal + lateFeeTotal;
              ...
              >
                <Card.Body>
                  ...
                  <h3>
                    ₹
                    {formatAmount(
                      Math.max(totalFeeAmount - donutChartSeries[0], 0)
                    )}
                  </h3>
                </Card.Body>
              </Card>
                  ...
                  {recentFeeEntries?.map((entry, index) => (
                    <tr key={entry?._id}>
                      ...
                      <td className="text-capitalize">
                        {entry?.student?.first_name || "Unknown"}
                      </td>
                      <td>
                        {entry?.student?.class_name?.class_name || "N/A"}
                      </td>
                      <td>
                        ₹
                        {formatAmount(
                      ...
                      <td>
                        {entry?.paymentMode?.payment_mode || "Online"}
                      </td>
                    </tr>
                  ))}
                </tbody>
