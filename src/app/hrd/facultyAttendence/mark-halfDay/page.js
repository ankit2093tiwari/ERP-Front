...
import { facultyHalfDayMark, getAllEmployee } from "@/Services";
...
const MarkHalfDay = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload = {
                ...
                half_day_staff: selectedStaffIds,
            };

            const res = await facultyHalfDayMark(payload);
            if (res.success) {
                toast.success("Half-day marked successfully!");
                setSelectedStaffIds([]);
                setSelectAll(false);
            } else {
                toast.error(res.message || "Failed to mark half-day.");
            }
        } catch (err) {
            toast.error(err?.response?.data.message || "Error while marking half-day.");
            console.error(err);
        }
    };
    ...
    const handleCopy = () => {
        ...
        const rows = staffList.map((row, index) =>
            `${index + 1}\t${row.employee_name}\t${row.category === "teaching" ? "Teaching" : "Non-Teaching"}\t${row.mobile_no}`
        );
